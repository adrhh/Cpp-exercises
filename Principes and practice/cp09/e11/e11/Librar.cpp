...
namespace Chrono
{
	...
	void Date::add_day(int n)
	{
		...
		while (tot_days)
		{
			...
			{
			case Date::jan:
				max_days = leapyear(y) ? 29 : 28;
				break;
			case Date::apr: case Date::jun: case Date::sep: case Date::nov:
				max_days = 30;
				break;
			}
			if (tot_days + d > max_days)
			{
				add_month(1);
				tot_days -= max_days;
			}
...
//--------------------------------------
//assumption for all years 1.01 = sunday
Date next_workday(const Date& d)
{
	Date temp = d;
	int days_formstart = 0;
	for (int i = 1; i < (int)d.month(); i++)
	{
		switch (i)
		{
		case Date::feb:
			days_formstart+= leapyear(d.year()) ? 29 : 28;
			break;
		case Date::apr: case Date::jun: case Date::sep: case Date::nov:
			days_formstart += 30;
		default:
	...
	days_formstart += d.day();
	days_formstart %= 7;
	Day day_ofweek = (Day)days_formstart;
	switch (day_ofweek)
	{
	case Day::friday:
		temp.add_day(3);
